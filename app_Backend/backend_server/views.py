...
def password_reset_request(request):
    if request.method == "POST":
        ...
        if user:  # If user exists
            ...
            email_template_name = "registration/password_reset_email.html"  # Template for the email body
            context = {
                "email": user.email,
                "domain": request.get_host(),  # Get the current domain
                "site_name": "Your Site",  # Your site name
                ...
                "protocol": "http",  # Protocol to be used in the email link
            }
            email_content = render_to_string(email_template_name, context)  # Render the email content
            try:
                send_mail(subject, email_content, settings.DEFAULT_FROM_EMAIL, [user.email], fail_silently=False)  # Send the email
                logger.info(f"Password reset email sent to {user.email}")
            except Exception as e:
                logger.error(f"Error sending password reset email: {str(e)}")
                return JsonResponse({"error": str(e)}, status=400)  # Handle email sending errors
            return JsonResponse({"message": "Password reset e-mail has been sent."}, status=200)  # Success response
        else:
            logger.warning(f"User with email {email} not found")
            return JsonResponse({"error": "User not found."}, status=404)  # User not found response
    logger.error("Invalid request method")
    return JsonResponse({"error": "Invalid request method."}, status=400)  # Invalid method response